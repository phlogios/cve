...
namespace CVE {
    ...
    void Renderer::initWindow(uint32_t width, uint32_t height) {
        glfwInit();
    ...
    void Renderer::initImgui() {
        //First set up vulkan stuff for imgui
        ...
        {
            //Todo: try doing this with the commandbuffer that is set up for the GUI
    ...
    void Renderer::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
        ...
        vkQueueWaitIdle(graphicsQueue);  //This could be done with vkWaitForFences instead,
        //which would allow for more simultaneous transfers
        //and more optimization opportunities for the driver
    ...
    void Renderer::createCommandPool(VkCommandPool* _commandPool, VkCommandPoolCreateFlags flags) {
    ...
    void Renderer::createCommandBuffers(VkCommandBuffer* commandBuffer, uint32_t commandBufferCount, VkCommandPool& commandPool) {
    ...
    void Renderer::createLogicalDevice() {
        ...
        std::set<uint32_t> uniqueQueueFamillies = { indices.graphicsFamily.value(), indices.presentFamily.value() };

        float queuePriority = 1.0f;
        for (uint32_t queueFamily : uniqueQueueFamillies) {
        ...
        // Device-specific validation layers are ignored in new implementations of Vulkan
        // So this code is here just to be sure that the validation layers work on older
        // implementations as well as new ones.
    ...
    bool Renderer::isDeviceSuitable(VkPhysicalDevice device) {
        ...
        std::cout << "Device type: " << (VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU ? "Discrete GPU" : "Other") << "\n";
        if (deviceFeatures.geometryShader) std::cout << "Geometry shader\n";
    ...
    VkSurfaceFormatKHR Renderer::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
        ...
        std::cout << "Did not find preferred swap surface format, settling on format " << availableFormats[0].format << " and colorspace " << availableFormats[0].colorSpace << "\n";
    ...
    VkExtent2D Renderer::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        /* Vulkan tells us to match the resolution of the window by setting
           the width and height in the currentExtent member. However, some
           window managers do allow us to differ here and this is indicated
           by setting the width and height in currentExtent to a special value:
    ...
    void Renderer::createSwapChain() {
        ...
        //store viewport and scissor members
        viewport.x = 0.0f;
        viewport.y = 0.0f;
    ...
    void Renderer::drawGUI(uint32_t imageIndex) {
    ...
    void Renderer::recreateSwapChain() {
        //it's possible that the number of swapchain images has changed
    ...
    void Renderer::cleanupSwapChain() {
        ...
        for (auto framebuffer : framebuffersGUI) {
            vkDestroyFramebuffer(device, framebuffer, nullptr);
        }


        vkFreeCommandBuffers(device, commandPool, static_cast<uint32_t>(commandBuffers.size()), commandBuffers.data());
        vkFreeCommandBuffers(device, commandPoolGUI, static_cast<uint32_t>(commandBuffersForGUI.size()), commandBuffersForGUI.data());
